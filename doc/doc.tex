...
\lambda &\ge 0 \notag
\end{align}
Because of the second condition in system \eqref{eq:posfirstordersystem}, based on a constant $\mu>0$, both $x$ and $\lambda$ are strictly positive and thus in the interior $S^\circ$.  (By contrast, in the general problem \eqref{eq:problem} the interior of the feasible set for the primal variables $x$ is different from $S^\circ$.)
...
\end{equation}
The first-order KKT conditions \cite[sections 14.4, 14.5]{GrivaNashSofer2009} of \eqref{eq:posproblem}, which one can derive using Lagrangian \eqref{eq:poslagrangian}, are nearly the same as \eqref{eq:posfirstordersystem} but with the second equation replaced by complementarity,
\begin{align}
...
\end{equation}
for multipliers $\tau\in\RR^m$ and $\lambda\in \RR^n$.  Function \eqref{eq:lagrangian} reduces to \eqref{eq:poslagrangian} if there are $m=0$ linear equality constraints and $\tau$ is absent.
...
\end{equation}
The Appendix gives a proof that the block matrix in \eqref{eq:newtonstep} is invertible.
...
\end{align}
Separate step sizes $\alpha_x,\alpha_\tau,\alpha_\lambda$ are used for the three vector variables $x,\tau,\lambda$, though this is not the only strategy in the literature \cite{YamashitaYabe1996}.  Because the constraint equations in \eqref{eq:problem} are linear, the full Newton step $\alpha_\tau=1$ is always used for the $\tau$ multipliers.
