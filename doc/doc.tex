...
Finally, regarding application of POPDIP to LP problems, one might ask how POPDIP does on the famously-worst-case-for-simplex Klee-Minty cube example \cite[chapter 9]{GrivaNashSofer2009}.  The answer for now is ``not any better than simplex.''  Compare the results of the \href{https://github.com/bueler/popdip/blob/main/matlab/kleeminty.m}{\texttt{kleeminty.m}} example code to the result of
...
FIXME
\begin{itemize}
\item add sufficient-decrease-type line search \cite[sections 19.3, 19.4]{NocedalWright2006} and the modified back-tracking line searches in \cite{BensonMunson2006}
\item Equations \eqref{eq:newtonstep} can be symmetrized by multiplying the third row by $-\Lambda_k^{-1}$:
\begin{equation}
\begin{bmatrix}
\grad^2 f(x) & -A^\top & - I \\
-A & 0 & 0 \\
...
-\grad f(x_k) + A^\top \tau_k + \lambda_k \\
A x_k - b \\
x_k - \mu_k \Lambda_k^{-1} e
\end{bmatrix}
 \label{eq:symmnewtonstep}
\end{equation}
This change allows application of symmetric linear solver techniques like Cholesky and MINRES \cite{Bueler2021}, but it also introduces the unbounded values $\lambda_k/x_k = \mu_k/x_k^2 \to +\infty$ into the calculation.  See \cite[section 19.3]{NocedalWright2006}.
\item reduce dimension of Newton step equations \cite[section 14.2]{NocedalWright2006}
