...
Next we consider an infinite-dimensional (continuum) optimization problem with positivity constraints.  Our problem is
\begin{equation}
\begin{matrix}
...
\end{equation}
over functions $u(x)$ with one well-defined derivative, on the interval $0\le x \le 1$, with a continuous source function $q(x)$ which is sometimes negative and sometimes positive.  When $q(x)>0$ then FIXME
